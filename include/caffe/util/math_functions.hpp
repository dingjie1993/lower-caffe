...
//new funtion to calculate char multiplication
...
void caffe_cpu_absmax( const int M, const int N,const Dtype* A,Dtype* B,
		bool ROW);

template <typename Dtype>
void caffe_cpu_dtchar( const int M, const int N, const Dtype* A, const Dtype* B,
		signed char* C,bool ROW);

template <typename Dtype>
void caffe_cpu_itdtype( const int M, const int N, const int* A, const Dtype* B,
		 const Dtype* C,Dtype* D);
